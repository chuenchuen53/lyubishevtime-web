...
const StyledTrigger = (props: { class?: string; children: JSXElement }) => {
  return (
    <Trigger
      class={twMerge(
        styles.trigger,
        "flex w-full items-center justify-between overflow-hidden rounded-lg border border-neutral-border bg-neutral-bg-container p-2.5 text-sm text-neutral-text transition-all data-[placeholder-shown]:text-neutral-text-tertiary",
        props.class,
      )}
    >
      {props.children}
...
const StyledContent = (props: { children: JSXElement }) => {
  return <Content class={`${styles.dropdown} z-10 rounded-lg bg-neutral-bg-elevated p-1 text-sm shadow-elevated`}>{props.children}</Content>;
};
